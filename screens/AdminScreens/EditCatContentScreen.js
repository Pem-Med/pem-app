...
const EditCatContentScreen = (props) => {
  ...
  const editedSelectedSubCategories = useSelector((state) => state.categoriesContent.categoriesContent.find((prod) => prod.id === subCategoryId),
  )

  const selectedCategory = useSelector((state) => state.categories.categories.find((prod) => prod.id === categoryId),
  )
